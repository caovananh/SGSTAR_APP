...
class About {
  ...
  Future<List<InfixMap>> fetchAboutServices(String token) async {
    ...
    final response = await http.get(Uri.parse(InfixApi.about),headers: Utils.setHeader(token.toString()));
  ...
  static Future<int> phonePermission(token) async{
    final response = await http.get(Uri.parse(InfixApi.currentPermission),headers: Utils.setHeader(token.toString()));
    var jsonData = json.decode(response.body);
