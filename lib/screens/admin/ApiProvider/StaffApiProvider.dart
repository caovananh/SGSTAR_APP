...
class StaffApiProvider{
  ...
  Future<StaffList> getAllStaff(int staffId) async {
    ...
    final response = await http.get(Uri.parse(InfixApi.getAllStaff(staffId)),headers: Utils.setHeader(token));
  ...
  Future<LeaveAdminList> getAllLeave(String url , String endPoint) async {
    ...
    print('$url $endPoint');
    final response = await http.get(Uri.parse(url),headers: Utils.setHeader(token));
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
      print(jsonData);
